...
class PipelineS3Input(PipelineInput):
    ...
    def run(self, data):
        ...
        print("Getting image from s3", self.config.src_path, "ID", data["unique_id"])
        data["image"] = self.pipeline.s3_client.get_image_from_s3(self.config.src_path, data["unique_id"])
