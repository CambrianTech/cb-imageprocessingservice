...
class PipelineLineFinder(PipelineStep):
    ...
    def run(self, data):
        ...
        def find_lines(image, min_length, use_lsd=False, refine=cv2.LSD_REFINE_NONE, scale=1.0, sigma_scale=1.0, quant=2.0, ang_th=22.5):
            min_length = int(min_length)
            if use_lsd:
                lsd = cv2.createLineSegmentDetector(refine, scale, sigma_scale, quant, ang_th)
